...
public final class LiquidGlassSwitchView: UIControl {
    ...
    private func setupViews() {
        ...
        trackTintLayer.backgroundColor = (onTintColor ?? UIColor.systemGreen).cgColor
        ...
        solidThumb.layer.cornerRadius = thumbSize / 2
    ...
    private func updateTrackColor() {
        trackTintLayer?.backgroundColor = (onTintColor ?? UIColor.systemGreen).cgColor
        if !isOn {
            trackView?.backgroundColor = offTintColor ?? UIColor(white: 0.9, alpha: 1.0)
        }
    }
    ...
    private func updateLayout() {
        let trackRange = trackWidth - thumbSize - 4.0
        let thumbX = 2.0 + (trackRange * thumbPosition)

        let scaledThumbSize = thumbSize * touchScale
        let thumbFrame = CGRect(
            x: thumbX + (thumbSize - scaledThumbSize) / 2,
            y: (trackHeight - scaledThumbSize) / 2,
            width: scaledThumbSize,
            height: scaledThumbSize
        )

        solidThumb.frame = thumbFrame
        solidThumb.layer.cornerRadius = scaledThumbSize / 2
    ...
    public override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        isInteracting = true
        morphToGlass()
        animateThumbScale(to: 1.2, duration: 0.25)
    }
    ...
    private func animateStateChange(to newState: Bool) {
        ...
        trackView.backgroundColor = newState ? (onTintColor ?? UIColor.systemGreen) : (offTintColor ?? UIColor(white: 0.9, alpha: 1.0))
...
private class GlassThumbView: UIView {
    ...
    private func setupMetal() {
        ...
        guard let library = try? device.makeLibrary(source: shaderSource, options: nil),
              ...
              let fragmentFunc = library.makeFunction(name: "thumbFragment"),
              let mtkView = self.metalView else {
            return
        }
    ...
    private func captureBackground() {
        guard let window = self.window, let device = self.device else { return }
...
private class GlassTrackView: UIView {
    ...
    private func setupMetal() {
        ...
        guard let library = try? device.makeLibrary(source: shaderSource, options: nil),
              ...
              let fragmentFunc = library.makeFunction(name: "trackFragment"),
              let mtkView = self.metalView else {
            return
        }
